...
extension StoreConfigKey where Value == Bool {
    /// 使用 box 包装共享状态 SharableState
    /// 配置这个为 true，即可在 SharableState 上正常使用 box() 方法
    /// 注意：这个配置为 true，代码你充分了解共享状态存储器生命周期管理，并已经重写了对应 shared 方法，自行管理存储器生命周期
    public static let useBoxOnShared: StoreConfigKey<Value> = .init("UseBoxOnShared")
}
