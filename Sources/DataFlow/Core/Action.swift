...
extension Store where State : ActionBindable {
    ...
    public func observeDefault<S:StateStorable, T:Equatable>(store: Store<S>, of keyPath: KeyPath<S, T>, callback: @escaping (_ new: T, _ old: T) -> State.BindAction) {
    ...
    public func observeDefault<S:StateStorable>(store: Store<S>, callback: @escaping (_ new: S, _ old: S) -> State.BindAction) {
        // 添加循环观察判断
